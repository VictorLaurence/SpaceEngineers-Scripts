...
namespace Scripts.ModeActive
{
    class ModeActive
    {
        ...
        void Main()
        {
            ...
            GridTerminalSystem.TimerBlock("Away Timer").Stop();

            if (GridTerminalSystem.Doors().AttemptEnable()) enabledSomething = true;
            if (GridTerminalSystem.GravityGenerators().AttemptEnable()) enabledSomething = true;
            if (GridTerminalSystem.Antennas().AttemptEnable()) enabledSomething = true;
            if (GridTerminalSystem.Lights().AttemptEnable()) enabledSomething = true;
            if (enabledSomething) GridTerminalSystem.SoundBlock("Sound Block - Lights On").Play();

            GridTerminalSystem.TimerBlock("Away Timer").Start();
        }
    }
