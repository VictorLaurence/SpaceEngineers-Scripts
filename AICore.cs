...
namespace Scripts.AICore
{
    class AICore
    {
        ...
        string broadcastKeyword = "Status Report";
        string broadcastExclusion = "Baluga";
        void Main()
        {
            var antenna = GridTerminalSystem.MyAntenna(broadcastKeyword, broadcastExclusion);
            if (antenna != null)
            {
                ...
                for (int x = 0; x < blocks.Count; x++)
                {
                    ...
                    if (inventoryOwner != null)
                    {
                        ...
                        {
                            var item = items[y];
                            if (item.Content.ToString().Contains("Ingot"))
                            {
                                if (item.Content.SubtypeName == "Uranium")
                                    uranium += item.Amount.RawValue;
                                else
                                    ingots += item.Amount.RawValue;
                            }
                            else if (item.Content.ToString().Contains("Ore"))
                                ore += item.Amount.RawValue;
                            else if (item.Content.ToString().Contains("Component"))
                                components += item.Amount.RawValue;
                        }
                    }
                }

                var message = new StringBuilder();
                message.AppendLine("=== Status Report ===");
                message.AppendLine("Hull: " + GridTerminalSystem.Integrity().ToString("0.0") + "%");
                message.AppendLine("Total Space: " + GridTerminalSystem.SpaceUsed().ToString("0.0") + "%");
                message.AppendLine("Uranium: " + Functions.RawToKg(uranium));
                message.AppendLine("Ingots: " + Functions.RawToKg(ingots));
                message.AppendLine("Ore: " + Functions.RawToKg(ore));
                message.AppendLine("Components: " + Functions.RawToKg(components));

                antenna.BroadcastMessage(message.ToString());
            }
        ...
        void Beep()
        {
            GridTerminalSystem.SoundBlock("Sound Block - Alert 1").Play();
        }
        void RandomBeep()
        {
            var rnd = new Random();
        ...
        void RandomLights()
        {
            ...
            {
                var light = lights[x] as IMyInteriorLight;
