...
namespace Scripts.FighterShip
{
    class FighterShip
    {
        ...
        string broadcastKeyword = "%";
        string broadcastExclusion = "Status Report";
        string myFaction = "Cage Industries";
        void Main()
        {
            //GridTerminalSystem.ApplyVirus(myFaction, broadcastKeyword, broadcastExclusion);

            var antenna = GridTerminalSystem.MyAntenna(broadcastKeyword, broadcastExclusion);
            if (antenna == null)
                antenna.BroadcastMessage("Hull " + GridTerminalSystem.Integrity().ToString("0.0") + "%");
